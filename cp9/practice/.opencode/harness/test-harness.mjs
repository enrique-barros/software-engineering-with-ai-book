...
console.log(
  "Test: un archivo .py con error debe lanzar el harness y revertirse con git restore",
);
await writeFile(target, "def foo(:\n", "utf-8");
let threw = false;
try {
...
check(
  restored === original,
  "el archivo quedo restaurado a su estado previo (git restore)",
);
...
check(
  log.includes("ROLLBACK=OK") && log.includes("git restore"),
  "el log contiene ROLLBACK=OK con metodo git restore",
);

console.log("Test: un archivo que no es .py no debe lanzar el harness");
await after({
  tool: "edit",
  args: { filePath: resolve(here, "opencode.json") },
});
check(true, "sin excepcion para archivo no .py");

console.log(
  "Test: un .py nuevo (sin seguimiento en git) con error debe eliminarse",
);
...
check(
  !(await exists(newFile)),
  "el archivo nuevo sin seguimiento fue eliminado",
);
...
try {
  await after({
    ...
    args: {
      edits: [
        { filePath: target },
        { filePath: resolve(here, "opencode.json") },
      ],
    },
  });
} catch {
...
check(
  (await readFile(target, "utf-8")) === original,
  "el archivo del multi_edit se restauro",
);
